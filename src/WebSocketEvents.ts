...
import { closeAllModals, errorModal } from "./ui.js";
const { wsUrl } = config;
...
export default class WebSocketEvents {
  ...
  constructor(
    ...
    wallet: string,
    infoCb: () => void,
    joystickCb: (data: joystickData) => void
  ) {
    this.wsEvents = new WebSocket(
      `${wsUrl}?datasource=${datasource}&wallet=${wallet}`
    );
    this.wsJoystick = new WebSocket(
      `${wsUrl}?datasource=${datasource}&joystick=true`
    );
    this.infoCb = infoCb;
  ...
  private onMessage(ev: any) {
    ...
    if (data.status === "success") {
      ...
      closeAllModals();
      errorModal(`Order ${data.orderId} has failed!`);
    }
  }
}
