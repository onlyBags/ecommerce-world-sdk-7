...
export const DetailItemModal = ({
  ...
  const updatePrice = async () => {
    ...
    const queryValues = selectedArticle?.attributes
      .map((attribute) => {
        const selectedValue = encodeURIComponent(selectedOptions[attribute.name])
    ...
    try {
      let response = await fetch(
        `https://ecommerce-api.dglive.org/v1/woocommerce/variation/1/${selectedArticle?.productId}?${queryAttributesAndValues}`,
        {
  ...
  return (
    <UiEntity
      ...
      uiText={{ value: selectedArticle.name, fontSize: 33, textAlign: 'top-center' }}
      ...
      >
        ...
        >
            ...
            {selectedArticle?.attributes.map((attribute, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <Dropdown
                    options={attribute?.options.flatMap((option) => option.value)}
                    onChange={(value) => {
          ...
          <UiEntity uiTransform={{ height: '31%', justifyContent: 'flex-end' }}>
            ...
            />
            <Label
              value={showVariablePrice && variablePrice !== '' ? `${variablePrice} ` : selectedOptions['price']}
              fontSize={29}
              textAlign="top-left"
