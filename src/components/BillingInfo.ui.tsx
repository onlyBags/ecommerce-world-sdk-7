...
export const BillingInfo = ({
  ...
  const validateBillingInfo = () => {
    hasErrors = false
    const { address1, city, state, country, postcode } = placeOrderDetails.billing
      ...
      >
        ...
        >
          ...
          <UiEntity uiTransform={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end' }}>
                ...
                return (
                  <UiEntity uiTransform={CONTAINER_BASE_PROPS} key={field.key}>
                    <Label
                      ...
                      textAlign="top-left"
                      color={
                        hasErrors && field.required && !placeOrderDetails.billing[field.key]
                          ? Color4.Red()
                          : Color4.Black()
                      }
            ...
            >
              {userData?.billingAddress.length > 0 && !hasSaveData && !hasSameShippingAddress && (
                <UiEntity
                  uiTransform={{
                    display: 'flex',
                    width: '80%'
              ...
              {hasUserData && userData?.billingAddress.length > 0 && (
                ...
                >
                  <Dropdown
                    options={userData?.billingAddress.map((address) => `${address.id}-${address.address1}`)}
                    ...
                    uiBackground={{
                      ...
                      texture: {
                        src: hasSameShippingAddress
                          ? 'images/contact/CheckBoxFilled.png'
                          : 'images/contact/CheckBoxEmpty.png'
                      }
                    }}
                  />
                  <UiEntity
                    ...
                    uiText={{ value: 'Same shipping address', color: Color4.fromHexString('#1C1C1C'), fontSize: 12 }}
                  />
                </UiEntity>
              )}
            </UiEntity>
          </UiEntity>
          <ModalFooter onBack={goBackToShipping} onContinue={validateBillingInfo} />
        </UiEntity>
