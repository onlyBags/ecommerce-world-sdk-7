...
import { Article, LineItem, PlaceOrderDetails, SelectedAttributes, WcOrderRes } from './types'
...
export async function postOrder(placeOrderDetails: PlaceOrderDetails) {
  try {
    ...
    if (!isApproved) {
      ...
      if (network !== '1') {
        return infoModal(`Please switch to Ethereum Mainnet, chainId: 1. \nYou are currently on chainId ${network}.`)
      } else {
        try {
          ...
          const message = `Error approving: ${error.message || 'Something went wrong, try again later.'}`
    ...
    const res = await http.post<WcOrderRes>('woocommerce/order', { datasourceId: 1, wcOrder })
    if (res.status === 200) {
      infoModal('Order placed successfully! \nNow you have 5 mins to pay the order.')
    ...
    errorModal('Error placing order: ' + error?.message || 'Something went wrong, try again later.')
  }
}

export const getVariationData = async (selectedArticle: Article, selectedAttributes: SelectedAttributes) => {
  ...
  const queryValues = selectedArticle?.attributes
    .map((attribute) => {
      const selectedValue = encodeURIComponent(selectedAttributes[attribute.name])
